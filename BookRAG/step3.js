...
const mainDir = join(__dirname, ".", "textbookdata");

const jsonFiles = readdirSync(mainDir).filter((item) =>
    statSync(join(mainDir, item)).isDirectory()
);
...
async function createCollection(category) {
    ...
    // lowercase category and replace spaces with dashes
    const collectionName = category.replace(/ /g, "");

    // delete collection if it exists
    const res = await weaviateClient.collections.delete(collectionName);
    // console.log("RESSSSS", res);

    await weaviateClient.collections.delete(collectionName.toLowerCase());
...
const processJSONs = async () => {
    ...
    for (const folder of jsonFiles) {
        ...
        const folderPath = join(mainDir, folder);
        ...
        // console.log("fileContent first", fileContent[0]);

        // const importPromises = fileContent.map((fileContent) =>
...
async function importData(category, files) {
    const collectionName = category.replace(/ /g, "");
    const collection = weaviateClient.collections.get(collectionName);

    console.log("importData", files.length);

    const result = await collection.data.insertMany(files);
    console.log("We just bulk inserted for", collectionName, result);
}
